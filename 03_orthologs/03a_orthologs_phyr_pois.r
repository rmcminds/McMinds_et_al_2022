...
ortholog_peptides <- lapply(genetrees, function(tree) {
...
ortholog_genes <- lapply(ortholog_peptides, \(x) ensembl2ext_full$ensembl_gene_id[match(x, ensembl2ext_full$ensembl_peptide_id)])
...
save.image(file.path(output_prefix,'03a_orthologs_phyr_results.RData'))
